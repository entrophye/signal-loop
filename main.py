...
from googleapiclient.http import MediaFileUpload
 ...
 SEEDS.read_text(encoding='utf-8').splitlines() if s.strip()]
 ...
 txt = random.choice(G_TEMPLATES).format(hook=hook, line1=line1,
 line2=line2, turn=turn)
 5
txt = ' '.join(txt.split())
 if len(txt) > max_len:
 txt = txt[:max_len-1] + '…'
 ...
 bg_path = random.choice(list(OVERLAYS.glob('*.jpg'))) if
 list(OVERLAYS.glob('*.jpg')) else None
 if not bg_path:
 ...
 clip =
 ImageClip(str(bg_path)).resize(height=1920).set_duration(duration)
 # gentle zoom
 w, h = clip.size
 zoom = clip.fx(crop, x1=0, y1=0, x2=w, y2=h).resize(lambda t: 1.02 +
 0.005*t)
 clip = zoom
 audio = AudioFileClip(voice_mp3)
 clip = clip.set_audio(audio)
 # subtitles
 txt = TextClip(textwrap.fill(text, 28), fontsize=64, color='white',
 font='DejaVu-Sans', method='caption', size=(1000, None), align='center')
 txt = txt.set_position(('center','center')).set_duration(audio.duration)
 result = CompositeVideoClip([clip, txt])
 result.write_videofile(out_mp4, fps=30, codec='libx264',
 audio_codec='aac')
 6
# ===== YouTube Upload =====
 def youtube_service():
...
a.export(mp3, format='mp3')
 ...
 title = f"SIGNAL — {n} — {random.choice(['The Pulse Beneath','Whisper 
Archive','Eclipsera'])}"
 desc = (
 ...
 tags += [t.strip() for t in os.getenv('CHANNEL_KEYWORDS').split(',')
 if t.strip()]
